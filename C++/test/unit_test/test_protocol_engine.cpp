...
void test_ping_triggers_ack(void) {
    ...
    engine->onPingRecv([&](uint8_t src) { pingCalled = true; });
    ...
    // ACK Frame: [FE][00][00][01][0A][02] [CRC=13]
...
void test_ack_callback(void) {
    ...
    engine->onAckRecv([&](uint8_t src) { ackCalled = true; });

    // ACK from 10 to Me
    // [FE][00][00][0A][01][02] [CRC=0D]
    std::vector<uint8_t> frame = {0xFE, 0x00, 0x00, 0x0A, 0x01, 0x02, 0x0D};
...
void test_system_report_serialization(void) {
    // Send System Report
    // Bat=3000mV (0x0BB8)
    engine->sendSystemReport(10, 1, 3000); // ID, Mode, Batt
...
void test_exec_sequence(void) {
    ...
    // Payload: [01] [00][00][04][01][E8][03][00][00]
...
void test_temp_hum_report_rx(void) {
    ...
    bool called = false;
    engine->onTempHumReportRecv([&](uint8_t src, float t, float h) {
        rxTemp = t;
        rxHum = h;
        called = true;
    });
...
void test_temp_hum_report_serialization(void) {
    engine->sendTempHumReport(10, 25.43f, 60.12f);
...
void test_pin_report_serialization(void) {
    engine->sendPinReport(10, 5, true);
    // Payload: [05][01]
...
void test_exec_sequence_serialization(void) {
    ...
    engine->sendExecSequence(10, steps);
...
void test_system_report_rx(void) {
    ...
    bool called = false;
    engine->onSystemReportRecv([&](uint8_t src, uint8_t mode, uint16_t batt) {
        received.mode = mode;
        received.batteryMv = batt;
        received.sourceId = src;
        called = true;
    });
...
void test_pin_report_rx(void) {
    ...
    bool rxVal = false;
    bool called = false;
    engine->onPinReportRecv([&](uint8_t src, uint8_t p, bool v) {
        rxPin = p;
        rxVal = v;
        called = true;
    });
...
void test_get_sensors_rx(void) {
    bool called = false;
    engine->onGetSensorsRecv([&](uint8_t src) { called = true; });

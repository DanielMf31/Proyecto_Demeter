...
def mock_devices_json(tmp_path):
    ...
    content = {
        "devices": {
            ...
            "valve": {
                "1": {"target_id": 1, "physical_pin": 10, "label": "Valvula 1"}
            }
        }
    }
...
def test_get_physical_mapping(mock_devices_json):
    ...
    # Test valvula valida (string ID)
    mapping = dm.get_physical_mapping("valve", "1")
    assert mapping == (1, 10)

    # Test valvula valida (int ID)
    mapping = dm.get_physical_mapping("valve", 1)
    assert mapping == (1, 10)

    # Test dispositivo inexistente
...
def test_translate_pin(mock_devices_json):
    ...
    # Traduccion de bomba (logical 1-4)
    assert dm.translate_pin(1) == (1, 4)
    assert dm.translate_pin(2) == (1, 5)

    # Traduccion de valvula (logical 5-8)

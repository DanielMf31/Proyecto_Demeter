...
from proyecto_demeter.client.ui.views import SidebarFrame, ControlPanelFrame, LogConsoleFrame, SequencePlannerFrame
from proyecto_demeter.client.ui.viewmodel import DemeterViewModel
from proyecto_demeter.shared.config.schemas import SequenceStep
...
class DemeterApp(ctk.CTk):
    ...
    def trigger_command(self, target_id, pin, action):
        """Bridge UI button -> Async VM"""
        if self.loop.is_running():
            asyncio.run_coroutine_threadsafe(self.vm.send_command(target_id, pin, action), self.loop)

...
from proyecto_demeter.shared.config.schemas import GpioCommand, ActionResponse, PingCommand, SequenceCommand, SequenceStep, GetSensorsCommand
...
class DemeterViewModel:
    ...
    async def send_command(self, target_id: int, pin: int, action: str):
        ...
        try:
            cmd = GpioCommand(target_id=target_id, pin=pin, action=action)

...
class TestProtocolMigration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open("Python/config/test_inv.json", "w") as f:
            f.write("""
            {
                "devices": {
            ...
            """)
        self.dev_mgr = DeviceManager("Python/config/test_inv.json")

    def tearDown(self):
        if os.path.exists("Python/config/test_inv.json"):
            os.remove("Python/config/test_inv.json")

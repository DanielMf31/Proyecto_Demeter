...
\textit{Parámetros:} $T$ = temperatura de bulbo seco (°C). \textit{Resultado:} kPa.
...
\[
VPD = \max\left(0,\; e_s - e_a\right) \qquad \text{(kPa)}
\]
...
\[
r = \frac{0{,}622 \cdot e_a}{P - e_a} \qquad \text{(kg agua/kg aire seco)}
\]
\[
h = 1{,}006 \cdot T + r \cdot (2501 + 1{,}86 \cdot T)
\qquad \text{(kJ/kg aire seco)}
\]

\textit{Parámetros:} $P$ = presión atmosférica (kPa, defecto 101,3~kPa = nivel del mar).
...
\qquad \text{(kPa)} \quad [\, LSD < 0 \Rightarrow \text{condensación probable}]
\]
...
\textit{Parámetros:} $T_{max}$ y $T_{min}$ son las temperaturas diarias extremas;
$T_{base}$ es la temperatura umbral de crecimiento (10\,°C por defecto para la mayoría
de hortícolas). La suma acumulada de GDD predice el estadio fenológico.
...
\textit{Parámetros:} $R_a$ = radiación extraterrestre en equivalente de mm/día
(defecto 15~mm/día, apropiado para latitud mediterránea media).
...
\[
DAP = \left\lfloor t_{sample} - t_{siembra} \right\rfloor \quad \text{(días)}
\]

...
Este documento explica cómo fluyen los mensajes en el sistema Demeter, diferenciando entre el método estandar (Redis) y la optimización actual (Directa).
...
> **Ventaja:** Permite escalar a infinitos servidores.
> **Desventaja:** Requiere un servicio extra (Redis) corriendo y configurado.
...
3.  Busca el cliente con ID nmo `raspberry_gateway`.
...
Este método es el que hemos activado para asegurar que tu sistema funcione sin depender de contenedores adicionales de infraestructura compleja por ahora.
